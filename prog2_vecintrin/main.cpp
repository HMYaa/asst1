...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_mask maskAll, maskExpPositive, maskCountPositive, maskClamp;
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    result = one;
    maskExpPositive = _cs149_init_ones(0);
    _cs149_vgt_int(maskExpPositive, exp, zero, maskAll);
    _cs149_vmove_float(result, x, maskExpPositive);

    _cs149_vsub_int(count, exp, oneInt, maskExpPositive);
    maskCountPositive = _cs149_init_ones(0);
    _cs149_vgt_int(maskCountPositive, count, zero, maskExpPositive);
    while (_cs149_cntbits(maskCountPositive) > 0) {
      _cs149_vmult_float(result, result, x, maskCountPositive);
      _cs149_vsub_int(count, count, oneInt, maskCountPositive);
      _cs149_vgt_int(maskCountPositive, count, zero, maskCountPositive);
    }
